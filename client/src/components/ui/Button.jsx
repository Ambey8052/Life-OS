...
export default function Button({
  ...
  return (
    <motion.button
      ...
      disabled={disabled}
      className={`inline-flex items-center justify-center gap-2 rounded-md px-4 py-2.5 min-h-[40px] text-sm font-medium transition-colors duration-150 disabled:opacity-50 disabled:pointer-events-none ${VARIANTS[variant]} ${className}`}
      {...props}
    >
      {children}
    </motion.button>
